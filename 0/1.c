...
int knapSackRec(int W, int wt[], int val[], int index, int dp[][50]) {
  // Base condition
  if (index < 0)
    return 0;
  if (dp[index][W] != -1)
...
int knapSack(int W, int wt[], int val[], int n) {
  // Allocate memory for the DP table dynamically
  int dp[n][50];  // Adjust the second dimension (50) based on your maximum weight
